/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Spring Boot 3.5
 *
 * Domain Model:
 * ${blueprint.getModelMetadata().getName()}
 *
 * Do not edit manually.
 ********************************************************
 * Turnstone Biologics Confidential
 *
 * 2026 Turnstone Biologics
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *       Turnstone Biologics - General Release
 */
package com.harbormaster.test;

import java.io.*;
import java.util.*;
import java.util.logging.*;

import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.assertNotNull;

import com.harbormaster.delegate.*;
import com.harbormaster.entity.*;
import com.harbormaster.api.*;

/**
 * Test Report class.
 *
 * @author    Harbormaster Dev Team
 */
public class ReportTest
{

// constructors

    public ReportTest()
    {
    	LOGGER.setUseParentHandlers(false);	// only want to output to the provided LogHandler
    }

// test methods
    @Test
    /** 
     * Full Create-Read-Update-Delete of a Report, through a ReportTest.
     */
    public void testCRUD() throws Throwable {        
        try {
        	LOGGER.info( "**********************************************************" );
            LOGGER.info( "Beginning full test on ReportTest..." );
            
            testCreate();            
            testRead();        
            testUpdate();
            testGetAll();                
            testDelete();
            
            LOGGER.info( "Successfully ran a full test on ReportTest..." );
            LOGGER.info( "**********************************************************" );
            LOGGER.info( "" );
        }
        catch( Throwable e ) {
            throw e;
        }
        finally  {
        	if ( handler != null ) {
        		handler.flush();
        		LOGGER.removeHandler(handler);
        	}
        }
   }

    /** 
     * Tests creating a new Report.
     *
     * @return    Report
     */
    public Report testCreate() throws Throwable {
        Report entity = null;

        LOGGER.info( "ReportTest:testCreate()" );
        LOGGER.info( "-- Attempting to create a Report");

        StringBuilder msg = new StringBuilder( "-- Failed to create a Report" );

        try {            
            entity = ReportService.getReportInstance().createReport( generateNewCommand() );
            assertNotNull( entity, msg.toString() );

            theId = entity.getReportId();
            assertNotNull( theId, msg.toString() + " Contains a null primary key" );

            LOGGER.info( "-- Successfully created a Report with primary key" + theId );
        }
        catch (Exception e)  {
            LOGGER.warning( unexpectedErrorMsg );
            LOGGER.warning( msg.toString() + entity );
            
            throw e;
        }
        return entity;
    }

    /** 
     * Tests reading a Report.
     *
     * @return    Report  
     */
    public Report testRead() throws Throwable {
        LOGGER.info( "ReportTest:testRead()" );
        LOGGER.info( "-- Reading a previously created Report" );

        Report entity = null;
        StringBuilder msg = new StringBuilder( "-- Failed to read Report with primary key" );
        msg.append( theId );

        try {
            entity = ReportService.getReportInstance().getReport( new ReportFetchOneSummary(theId) );
            
            assertNotNull( entity,msg.toString() );

            // for use later
            theId = entity.getReportId();
            
            LOGGER.info( "-- Successfully found Report " + entity.toString() );
        }
        catch ( Throwable e ) {
            LOGGER.warning( unexpectedErrorMsg );
            LOGGER.warning( msg.toString() + " : " + e );
            
            throw e;
        }

        return( entity );
    }

    /** 
     * Tests updating a Report.
     *
     * @return    Report
     */
    public Report testUpdate() throws Throwable {
        LOGGER.info( "ReportTest:testUpdate()" );
        LOGGER.info( "-- Attempting to update a Report." );

        StringBuilder msg = new StringBuilder( "Failed to update a Report : " );        
        Report entity = null;
    
        try {            
        	UpdateReportCommand updateCommand = this.generateUpdateCommand();
        	
        	// apply the current id as to update the fields of the current entity
        	updateCommand.setReportId( theId );
            
            assertNotNull( updateCommand, msg.toString() );

            LOGGER.info( "-- Now updating the created Report." );
            
            ReportService proxy = ReportService.getReportInstance();            
            entity = proxy.updateReport( updateCommand );   
            
            assertNotNull( entity, msg.toString()  );

            LOGGER.info( "-- Successfully saved Report - " + entity.toString() );
        }
        catch ( Throwable e ) {
            LOGGER.warning( unexpectedErrorMsg );
            LOGGER.warning( msg.toString() + " : primarykey-" + theId + " : entity-" +  entity + " : " + e );
            
            throw e;
        }

        return( entity );
    }

    /** 
     * Tests deleting a Report.
     */
    public void testDelete() throws Throwable {
        LOGGER.info( "ReportTest:testDelete()" );
        LOGGER.info( "-- Deleting a previously created Report." );
        
        try {
        	DeleteReportCommand deleteCommand = new DeleteReportCommand( theId );
        	
            ReportService.getReportInstance().delete( deleteCommand );
            
            LOGGER.info( "-- Successfully deleted Report with primary key " + theId );            
        }
        catch ( Throwable e ) {
            LOGGER.warning( unexpectedErrorMsg );
            LOGGER.warning( "-- Failed to delete Report with primary key " + theId );
            
            throw e;
        }
    }

    /** 
     * Tests getting all Reports.
     *
     * @return    Collection
     */
    public List<Report> testGetAll() throws Throwable {    
        LOGGER.info( "ReportTest:testGetAll() - Retrieving Collection of Reports:" );

        StringBuilder msg = new StringBuilder( "-- Failed to get all Report : " );        
        List<Report> collection  = null;

        try {
            // call the static get method on the ReportService
            collection = ReportService.getReportInstance().getAllReport();

            if ( collection == null || collection.size() == 0 ) {
                LOGGER.warning( unexpectedErrorMsg );
                LOGGER.warning( "-- " + msg.toString() + " Empty collection returned."  );
            }
            else {
	            // Now print out the values
	            Report currEntity  = null;            
	            Iterator<Report> iter = collection.iterator();
					
	            while( iter.hasNext() ) {
	                // Retrieve the entity   
	                currEntity = iter.next();
	                
	                assertNotNull( currEntity,"-- null value object in Collection." );
	                assertNotNull( currEntity.getReportId(), "-- value object in Collection has a null primary key" );        
	
	                LOGGER.info( " - " + currEntity.toString() );
	            }
            }
        }
        catch ( Throwable e ){
            LOGGER.warning( unexpectedErrorMsg );
            LOGGER.warning( msg.toString() );
            
            throw e;
        }

        return( collection );
    }
    
    public ReportTest setHandler( Handler handler ) {
    	this.handler = handler;
    	LOGGER.addHandler(handler);	// assign so the LOGGER can only output results to the Handler
    	return this;
    }
    

	/**
	 * Returns a new populated Report
	 * 
	 * @return CreateReportCommand alias
	 */
	protected CreateReportCommand generateNewCommand() {
        CreateReportCommand command = new CreateReportCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  ReportStatus.values()[0] );
		
		return( command );
	}

		/**
		 * Returns a new populated Report
		 * 
		 * @return UpdateReportCommand alias
		 */
	protected UpdateReportCommand generateUpdateCommand() {
	        UpdateReportCommand command = new UpdateReportCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  null,  new HashSet<>(),  new HashSet<>(),  new HashSet<>(),  new HashSet<>(),  new HashSet<>(),  ReportStatus.values()[0] );
			
			return( command );
		}
	//----
    
// attributes 

    protected UUID theId  = null;
	private final Logger LOGGER = Logger.getLogger(Report.class.getName());
	private Handler handler = null;
	private String unexpectedErrorMsg = ":::::::::::::: Unexpected Error :::::::::::::::::";
}
